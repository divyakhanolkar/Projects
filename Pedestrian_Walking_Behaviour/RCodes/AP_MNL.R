...
# Question 1: MNL for Predicting Pedestrain Choices ----------------------------

apollo_initialise()

apollo_control=list(modelName  ="M1",
                    modelDescr ="Model to Predict Pedestrian choices: Multinomial Logit",
                    indivID    ="InvID")
...
# Question 2 - Aggregate probability of all utilities---------------------------------

forecast <- apollo_prediction(M1, apollo_probabilities, apollo_inputs)

# Question 3 -  probabilities of individual utilities-----------------------------------

#csvFilePath <- "Subject3.csv"

# Append the new data to the existing CSV file
#existingData <- read.csv(csvFilePath)

# Create new columns
#existingData <- cbind(existingData, forecast)

# Write the updated data frame back to the CSV file
#write.csv(existingData, file = csvFilePath, row.names = FALSE)